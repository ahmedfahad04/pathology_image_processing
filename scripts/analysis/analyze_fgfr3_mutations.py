...
What this script does:
  1. Scans the local `data/` folder produced by `scripts/analysis/fetch_cbioportal_data.py`
...
Usage:
  python scripts/analysis/analyze_fgfr3_mutations.py
...
def main():
    ...
    if len(per_image_dirs) == 0:
        print(f"[ERROR] No per-image folders in {data_dir}. Did you run scripts/analysis/fetch_cbioportal_data.py?")
        return
...
---
*Generated by `scripts/analysis/analyze_fgfr3_mutations.py` on {pd.Timestamp.now().strftime('%Y-%m-%d %H:%M')}.  Paper DOI: 10.1038/s41467-024-55331-6*
"""
